...
$(function () {
		...
		if(str!="login.htm"){
			if(user==null){
				...
				window.identity = user.identity;
			}
		}
		var topHTML = localStorage.getItem("top");
		var bottomHTML = localStorage.getItem("bottom");
		if(topHTML==null){
			$.ajax({ url: "../../view/Public/top.htm", context: document.body,async:false, success: function(result){
			localStorage.setItem("top",result);
            $("#top").html(result);
            Top.init();
        	}});
			$.ajax({ url: "../../view/Public/left.htm", context: document.body,async:false, success: function(result){
				localStorage.setItem("bottom",result);
				$("#left").html(result);
				InitiateSideMenu();
				InitiateWidgets();
			}});
